...
# Render URL (Important for Web App)
...
# --- FLASK SERVER & EDITOR HTML ---
app = Flask(__name__)

# This is the Code Editor UI (HTML/JS/CSS)
EDITOR_HTML = """
<!DOCTYPE html>
<html>
<head>
    ...
    <script>
        ...
        function saveCode() {
            ...
            fetch('/save_code', {
                ...
                body: JSON.stringify({ target_id: targetId, code: newCode })
            })
            .then(response => response.json())
            .then(data => {
                if(data.status === 'success') {
                    tg.showAlert("✅ Saved & Redeployed!");
                    tg.close();
                } else {
...
def editor_page():
    ...
    if uid != ADMIN_ID and uid != owner:
        return "⛔ Access Denied: You do not own this file."
    ...
    return render_template_string(EDITOR_HTML, code=code_content, target_id=target_id, filename=target_id)
...
def save_code_route():
    ...
    try:
        ...
        # 2. Restart Process (Background Logic)
...
def restart_process_background(target_id):
    # Logic to kill and restart process from Flask thread
...
async def manage_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("man_"):
        ...
        # WEB APP BUTTON - The Magic Part 
        web_app_url = f"{BASE_URL}/editor?id={tid}&uid={uid}"
        btns.append([InlineKeyboardButton("✏️ Live Editor", web_app=WebAppInfo(url=web_app_url))])
        
        btns.append([InlineKeyboardButton("📦 Update Reqs", callback_data=f"edit_req_{tid}"), InlineKeyboardButton("🔐 Env Vars", callback_data=f"edit_env_{tid}")])
