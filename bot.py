...
<head>
    ...
    <style>
        ...
        .header { padding: 10px; background: #44475a; display: flex; justify-content: space-between; align-items: center; }
...
<body>
    <div class="header">
        <h3>✏️ Edit: {{ filename }} ({{ file_type }})</h3>
        <button class="btn" onclick="saveCode()">💾 Save & Deploy</button>
    </div>
    <textarea id="code_area">{{ code }}</textarea>
    <script>
        var tg = window.Telegram.WebApp;
        tg.expand(); 
        var mode = "{{ file_type }}" === "src" ? "python" : "shell";
...
def editor_page():
    ...
    file_type = request.args.get('type', 'src') # src, env, req
    ...
    if file_type == 'env':
        # Logic to find env path handled in resolve, but we need exact path here
...
def restart_process_background(target_id):
    ...
    proc = subprocess.Popen(["python", "-u", script_path], env=custom_env, stdout=log_file, stderr=subprocess.STDOUT, cwd=work_dir, preexec_fn=os.setsid)
...
async def receive_env_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX: Added response logic here
    if context.user_data.get('type') == 'repo': 
        await update.message.reply_text("✅ **Variables Saved.**", reply_markup=git_extras_keyboard())
        return WAIT_GIT_EXTRAS
    
    await update.message.reply_text("✅ **Variables Saved.**", reply_markup=extras_keyboard())
    return WAIT_EXTRAS
...
async def manage_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("man_"):
        ...
        # Row 1: Stop/Run | URL
        r1 = [InlineKeyboardButton("🛑 Stop", callback_data=f"stop_{tid}")] if is_running else [InlineKeyboardButton("🚀 Run", callback_data=f"rerun_{tid}")]
        if is_running: r1.append(InlineKeyboardButton("🔗 URL", callback_data=f"url_{tid}"))
        
        # Row 2: Live Editors (Source, Env)
        r2 = [
            InlineKeyboardButton("✏️ Edit Code", web_app=WebAppInfo(url=f"{BASE_URL}/editor?id={tid}&type=src&uid={uid}")),
            InlineKeyboardButton("✏️ Edit Env", web_app=WebAppInfo(url=f"{BASE_URL}/editor?id={tid}&type=env&uid={uid}"))
        ]
        
        # Row 3: Live Editor (Reqs) | Logs
        r3 = [
             InlineKeyboardButton("✏️ Edit Reqs", web_app=WebAppInfo(url=f"{BASE_URL}/editor?id={tid}&type=req&uid={uid}")),
             InlineKeyboardButton("📜 Logs", callback_data=f"log_{tid}")
        ]
        
        # Row 4: Delete
        r4 = [InlineKeyboardButton("🗑️ Delete", callback_data=f"del_{tid}")]

        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([r1, r2, r3, r4]), parse_mode="Markdown")
